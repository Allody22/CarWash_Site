...
import {createTireOrder, getActualTireOrders, getPrice} from "../http/orderAPI";
...
const CreatingTireOrder = () => {
    ...
    const [tireOrders, setTireOrders] = useState([]);
    ...
    const handleCreateOrder = async (e) => {
        e.preventDefault();
        try {
            const response = await createTireOrder(selectedItems.map(i => i.replace(/ /g, '_')),
                userContacts, wheelR, startTime.toISOString(), endTime.toISOString(),
    ...
    const handleGetOrders = async (event) => {
        ...
        try {
            ...
            setTireOrders(response.orders.map(item => item.replace(/_/g, ' ')))
        } catch (error) {
            console.error(error);
        }
    };
    ...
    return (
        <>
            ...
            <div className="label-container">
                <div className="price-label">Цена услуг: {price ? price : "Цена этих услуг не известна"}</div>
                <div className="order-time-label">Время выполнения:
                    {orderTime ? orderTime : "Время выполнения этих услуг не известно"}</div>
            </div>
